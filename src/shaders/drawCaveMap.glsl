...
varying vec2 v_uv;
    ...
    void main()
    {
        ...
        lookup.x = 1. - lookup.x;
        lookup.y = 1. - lookup.y;
        vec2 normalRocks2d = 2.*texture2D(u_normalRocks, 3.0*(lookup + .02*surfDir*surfDepth)).rg - 1.;
        normalRocks2d *= -1.;
        vec3 normalRocks = normalize(vec3(normalRocks2d, 1. - length(normalRocks2d)));

        normalRocks = axisAngle(normalize(vec3(surfDir.y, -surfDir.x, 0)), 5.*surfDepth) * normalRocks;
